...
def tokenizing_distributed_data_loader(B, T, split, device, tokenizer_threads=4, tokenizer_batch_size=128):
    ...
    while True:
        ...
        inputs = inputs_cpu.view(B, T).to(device=device, dtype=torch.int32, non_blocking=is_cuda)
        targets = targets_cpu.view(B, T).to(device=device, dtype=torch.int64, non_blocking=is_cuda)
